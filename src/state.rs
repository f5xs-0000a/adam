...
impl AdamState {
    pub fn new(initial_vectors: Vec<Vec<FLOAT>>)
    -> Result<AdamState, Error> {
        assert_equal_parameter_count(&initial_vectors)?;
    ...
    pub fn get_vectors(&'a self) -> &'a [Vec<FLOAT>] {
        &self.vectors
    }

    pub fn update_vectors(&mut self, vectors: Vec<Vec<FLOAT>>)
    -> Result<(), Error> {
    ...
    pub fn update(&mut self, scores: Vec<FLOAT>, params: &AdamParams)
    -> Vec<Vec<FLOAT>> {
        self.t += 1;
        ...
        let m_hat = get_bias_corrected_moment_estimate(
            &self.m,
            params.beta_1,
            self.t
        );

        let v_hat = get_bias_corrected_moment_estimate(
            &self.v,
            params.beta_2,
            self.t,
        );
...
fn assert_equal_parameter_count(
    vectors: impl Iterator<Item = &Vec<FLOAT>>,
) -> Result<(), Error> {
...
fn assert_equal_parameter_count_given_count(
    count: usize,
    vectors: impl Iterator<Item = &Vec<FLOAT>>,
) -> Result<(), Error> {
...
fn generate_gradients(
    param_iter: impl Iterator<Item = &[FLOAT]>,
    ...
    let mut x_sums = param_iter
        .next()
        .map(|vec| vec.to_vec());

    // if we have no set of parameters in the first place
    if x_sums.is_none() {
        return vec![];
    }
    ...
    // sum all the y's
    let mut y_counter = 0;
    let rise = scores_iter
        .inspect(|_| y_counter += 1)
        .sum::<FLOAT>();
